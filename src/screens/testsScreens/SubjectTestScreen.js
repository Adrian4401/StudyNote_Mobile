...
export default function SubjectTestScreen() {
  ...
  const questionTypeOptions = [
    {
      label: getTranslatedText("trueFalseQuestionType"),
      value: "true_false",
      icon: "toggle-switch-outline",
    },
    {
      label: getTranslatedText("singleChoiceQuestionType"),
      value: "single_choice",
    ...
    {
      label: getTranslatedText("openQuestionType"),
      value: "open",
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const subjectsData =
          await getAllSubjects(userToken);
  ...
  const filteredNotes = useMemo(() => {
    ...
    return notes.filter(
      (note) =>
        String(note.subject_id ?? note.subjectId) ===
        ...
        >
          ...
          >
            <MaterialCommunityIcons
              ...
              size={22}
              color={
                selected ? "#fff" : theme.textSecondary
              }
    ...
    <SafeareaNoNav>
      <View style={styles.headerBackground}>
        <Text style={styles.headerText}>
          {getTranslatedText("testGeneratorTitle")}
        </Text>
      </View>
